...
export default class BedsAndBathsFilter extends React.Component {
  ...
  render() {
    ...
    let bedroom_min = filters.bedroom_min || 0;
    let baths_min = filters.baths_min || 0;
    return (
      <div>
        <a href="javascript:;" className={(active) ? 'active' : ''} onClick={onClick}>
          {(() => {
            ...
            if(active){
              if(bedroom_min){
                value += `Beds ${bedroom_min} +`;
              }
              if(baths_min){
                value += ((value.length) ? ' & ' : '') + `Baths ${baths_min} +`;
              }
              return value;
            }else{
              return 'Beds & Baths';
                ...
                <div className="col-xs-4">
                  <div className="clearfix">
                    <IncrementCounter
                      ...
                      minDisabledAt={0}
                      maxDisabledAt={20}
                      updateCounter={(val) => this.updateFilter('bedroom_min', parseInt(val))}
                    />
                  </div>
                </div>
              </div>
                ...
                <div className="col-xs-4">
                  <div className="clearfix">
                    <IncrementCounter
                      ...
                      minDisabledAt={0}
                      maxDisabledAt={20}
                      updateCounter={(val) => this.updateFilter('baths_min', parseInt(val))}
                    />
                  </div>
                </div>
              </div>
